...
public class Service {
    ...
    public void countPopularWordsWithGoogleFreq(Map<Path, Map<String, Long>> dirToRes, Map<String, Long> wordToGoogleFreq) {
...
//                .collect(toMap(Map.Entry::getKey, e -> getGoogleWordFrequency(e.getKey())))
        ...
        dirToRes.forEach((k, v) -> v.entrySet().parallelStream().forEach(e -> {
            Long value = wordToGoogleFreq.get(e.getKey());
            if(value == null) {
                wordToGoogleFreq.put(e.getKey(), getGoogleWordFrequency(e.getKey()));
            }
            e.setValue(value == null ? wordToGoogleFreq.get(e.getKey()) : value);
        }));
    }
